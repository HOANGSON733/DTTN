...
const DetailsMovie = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { details, category, country, episodes } = await getDetailMovie(
          slug
        );
        setDetails(details);
        ...
        setCommentUrl(
          `https://netflixnhs.vercel.app/movie/detailsmovie/${slug}`
        );
                    ...
                    <div className="watch">
                      ...
                      {details.trailer_url === "" ? (
                        ...
                        <div className="trailer" onClick={handleShowModal}>
                          <button>
                            Trailer <i className="fa-brands fa-youtube"></i>
                          </button>
                        </div>
                      )}
                      {episodes.length > 0 ? (
                        <div className="watch_now">
                          <Link
                            to={`/movie/detailsmovie/watch/${details.slug}`}
                          >
                            <button>
                              Xem Phim <i className="fa-solid fa-play "></i>
                            </button>
                          </Link>
                        </div>
                      ) : (
                    ...
                    <div className="info">
                      {details.status && (
                        <div className="info_item">
                          <h5>
                            {" "}
                            Đạo Diễn: <p>{details.director}</p>
                          </h5>
                          <h5>
                            Diễn viên:<p>{details.actor + ""}</p>
                          </h5>

                          <h5>
                            Tập phim:<p>{details.episode_current}</p>
                          </h5>
                          <h5>
                            Thời lượng:<p>{details.time}</p>
                          </h5>
                        </div>
                      )}
                      {details.country && (
