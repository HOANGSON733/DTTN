...
const FilterMovie = () => {
  ...
  const filteredMovies = filterMovie.filter(
    (movie) =>
      (genresFilter
        ? movie.category.some((item) => item.slug === genresFilter)
        : true) &&
      (countryFilter
        ? movie.country.some((item) => item.slug === countryFilter)
        : true) &&
  ...
  const currentPageData = filteredMovies
    ? filteredMovies.slice(offset, offset + itemsPerPage)
    : [];
